...
function renderKabayans(kabayans) {
  ...
  kabayans.forEach(pkg => {
    const image = pkg.main_image || "../assets/icons/no-image.png";
    let incs = [];
    let excs = [];
    ...
    card.innerHTML = `
      ...
      <div class="tour-inclusions">
        ${incs.map(i => `<div class="inclusion-item"><img src="../assets/icons/check.png"><span>${i}</span></div>`).join("")}
      </div>
      <div class="tour-exclusions">
        ${excs.map(e => `<div class="inclusion-item"><img src="../assets/icons/x.png"><span>${e}</span></div>`).join("")}
      </div>
